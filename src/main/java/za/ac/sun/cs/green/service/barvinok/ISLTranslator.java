...
public class ISLTranslator {
	...
	private void constructBounds(ArrayList<IntVariable> vars, ArrayList<IntVariable> bounds) {
		for (IntVariable x : vars) {
			IntVariable upper = new IntVariable(x.getCachedString() + "min", 0, 0);
			IntVariable lower = new IntVariable(x.getCachedString() + "max", 0, 0);
	...
	private String addBounds(ArrayList<IntVariable> vars, ArrayList<IntVariable> bounds) {
		...
		for (IntVariable var : vars) {
			bound.append(" and (").append(bounds.get(j).getCachedString()).append(" <= ").append(var.getCachedString()).append(" <= ").append(bounds.get(j + 1)).append(")");
			j = j + 2;
		}
